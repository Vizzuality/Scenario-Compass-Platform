 ...
 * @property {boolean} highFeasibility True if run has any "high" severity Feasibility Vetting flags.
 * @property {boolean} mediumFeasibility True if run has any "medium" severity Feasibility Vetting flags.
 * @property {boolean} highSustainability True if run has any "high" Sustainability flags.
 * @property {boolean} mediumSustainability True if run has any "medium" Sustainability flags.
 */
 ...
 * ```typescript
 * const flagMetas = [
 *   { key: "Feasibility Concern|Solar PV", value: "high" },
 *   { key: "Sustainability Concern|Wind Turbine", value: "medium" }
 * ];
 *
 ...
 * ```
 */
const _analyzeFlagsForSingleRun = (flagMetas: Array<ShortMetaIndicator>): RunFlagAnalysis => {
  ...
  flagMetas.forEach((meta) => {
    ...
    if (meta.key.startsWith(FEASIBILITY_META_KEY)) {
      ...
      if (value === VALUE_MEDIUM) flags.mediumFeasibility = true;
    }

    if (meta.key.startsWith(SUSTAINABILITY_META_KEY)) {
      if (value === VALUE_HIGH) flags.highSustainability = true;
      if (value === VALUE_MEDIUM) flags.mediumSustainability = true;
    }
  });
...
export const getFlagCategoryForRun = (metaIndicators: Array<ShortMetaIndicator>): CategoryKey => {
  const flagMetas = _filterFlagMetaIndicators(metaIndicators);
  const flags = _analyzeFlagsForSingleRun(flagMetas);
