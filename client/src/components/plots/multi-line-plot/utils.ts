...
export const renderMultiLinePlot = ({
  ...
  lines
    ...
    .on("mousemove", function (event, run) {
      ...
      const tooltipHTML = `
       <ul style="list-style-type: disc; margin: 0; padding-left: 20px; display: flex; flex-direction: column; gap: 3px; color: black;">
          ...
          <li><strong>Sceanrio:</strong> <span>${run.scenario.name}</span></li>
      </ul>
    `;
