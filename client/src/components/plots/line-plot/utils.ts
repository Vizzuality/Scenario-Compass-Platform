...
import { DataPoint, ProcessedData } from "@/components/plots/types/plots";
...
export const createValueBasedColorMapping = (scenarios: Map<string, DataPoint[]>) => {
  // Calculate average value for each scenario
  ...
  // Sort scenarios by their average values
  const sortedScenarios = Array.from(scenarioAverages.entries())
    .sort((a, b) => a[1] - b[1]) // Sort by average value (ascending)
    .map(([scenarioKey]) => scenarioKey);

  // Create custom interpolation function using lighter theme colors
  const createCustomInterpolation = () => {
    // Define your color stops with burgundy as the darkest
    const colorStops = [
      { position: 0.0, color: "#C5C1F7" }, // Lilac Light (lowest values)
      { position: 0.25, color: "#B3AEF5" }, // Lilac Dark
      { position: 0.5, color: "#8077EE" }, // Lilac Darkest
      { position: 0.75, color: "#E0B8C3" }, // Burgundy Light
      { position: 1.0, color: "#652C3B" }, // Burgundy (highest values)
    ];
