...
import { useScenarioDashboardMetaIndicatorsFilterUrl } from "@/hooks/use-scenario-dashboard-meta-indicators-filter-url";
 ...
 * @since 1.0.0
 */
export function useFilterPointsPipeline(variable: string): MetaDataPoint[] {
  // Extract filter parameters from URL state
  const { year, endYear, startYear, geography } = useScenarioDashboardUrlParams();
  ...
  return useMetaIndicatorsFilter({
